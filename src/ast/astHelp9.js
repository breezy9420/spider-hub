...
function encryptStr_u(path, name) {
        ...
        if (binding) {
            ...
            for (const p of referencePaths) {
                if (types.isCallExpression(p.parentPath)) {
                    ...
                    const plainText = i(param1.value);
                    try {
                        p.parentPath.replaceWith(types.stringLiteral(plainText));
                        console.log(`${leftName}('${param1.value}') => ${plainText}`);
                    } catch (e) {
                        console.error(`${leftName}('${param1.value}') => ${plainText}`);
                    }
                } else {
                    encryptStr_u(p.parentPath, leftName);
                }
            }
        }
    }
...
async function main() {
    const encryptJsPath = path.resolve(path.dirname(fileURLToPath(import.meta.url)), "./page9.js");
    ...
    const outputJsPath = path.resolve(path.dirname(fileURLToPath(import.meta.url)), "./page9_decrypt.js");
